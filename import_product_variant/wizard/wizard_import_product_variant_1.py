...
class ImportProductVariant(models.TransientModel):
    ...
    def import_button(self):
        ...
        product_attr_line_obj = self.env['product.template.attribute.line']
        ...
        barcodes_columns = []
        for col in columns:
            if self.check_barcode_column(col):
                barcodes_columns.append(self.get_barcod_column(col))
        ...
        for line in archive_lines:
            ...
            list_bar_line = self.get_variants_attribute_line(line, barcodes_columns)
            bar_values=[]
            for p in list_bar_line:
                x=p.split(':')[1]
                bar_values.append(x)
    ...
    def get_attributes(self, product_name, archive_lines, attributes_columns):
        ...
        for line in archive_lines:
            if line.get('name',"") == product_name:
                for attr_col in attributes_columns:
                    if str(line.get(attr_col,"")).strip() != "":
    ...
    def get_variants_attributes(self, product_name, archive_lines, attributes_columns):
        ...
        for line in archive_lines:
            if line.get('name',"") == product_name:
                for attr_col in attributes_columns:
                    if str(line.get(attr_col,"")).strip() != "":
                        lis.append(attr_col.split(':')[1].strip()+':'+str(line.get(attr_col,"")).strip())
        lis = list(set(lis))
        return lis
    ...
    def get_variants_attribute_line(self, line, attributes_columns):
        ...
        for att_col in attributes_columns:
            ...
            if at != "" and at != None and at != False and at != " ":
                lis.append(att_col.split(':')[1].strip()+":"+str(at))
        return lis
    ...
    def get_variants_from_column(self, archive_lines, attrib):
        ...
        for variant in lis_variant:
            lis.append(attrib.split(':')[1].strip()+':'+ str(variant))
        return lis
    ...
    @api.model
    def check_barcode_column(self, col):
        if col != "" and col != None and col != False and col != " " and col.count(':') > 0:
            cad = col.split(':')
            if cad[0].count('b') > 0 and cad[0].isalnum():
                if cad[1] != "" and cad[1] != None and cad[1] != False and cad[1] != " ":
                    return True
        return False

    @api.model
    def get_barcod_column(self, col):
        if col != "" and col != None and col != False and col != " " and col.count(':') > 0:
            cad = col.split(':')
            if cad[0].count('b') > 0 and cad[0].isalnum():
                if cad[1] != "" and cad[1] != None and cad[1] != False and cad[1] != " ":
