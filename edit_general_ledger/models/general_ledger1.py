...
class report_account_general_ledger(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        ...
        return [{'name': ''},
                ...
                {'name': _("PDebit"), 'class': 'number'},
                {'name': _("PCredit"), 'class': 'number'},]

    @api.model
    def _get_lines(self, options, line_id=None):
        print("***this is fun  _get_lines ** ")
        ...
        grouped_accounts = self.with_context(date_from_aml=dt_from, date_from=dt_from and
            ...
            options, line_id)
        print("grouped_accounts == ", grouped_accounts)
        ...
        for account in sorted_accounts:
            ...
            if options.get('filter_accounts'):
                ...
                if not any([display_name_part.startswith(options.get('filter_accounts')) for display_name_part in
                            display_name.split(' ')]):
                    continue
            debit = grouped_accounts[account]['debit']
            pdebit = grouped_accounts[account]['debit'] - grouped_accounts[account]['initial_bal']['debit']
            credit = grouped_accounts[account]['credit']
            pcredit = grouped_accounts[account]['credit'] - grouped_accounts[account]['initial_bal']['credit']
                ...
                lines.append({
                    ...
                    'columns': [{'name': v} for v in
                                [amount_currency, self.format_value(debit), self.format_value(credit),
                                 self.format_value(balance), self.format_value(pdebit), self.format_value(pcredit)]],
                    'level': 2,
                    'unfoldable': True,
            ...
            if 'account_%s' % (account.id,) in options.get('unfolded_lines') or unfold_all:
                ...
                for line in amls:
                    if options.get('cash_basis'):
                        ...
                        line_pdebit = line.debit_cash_basis
                        line_credit = line.credit_cash_basis
                        line_pcredit = line.credit_cash_basis
                    else:
                        line_debit = line.debit
                        line_pdebit = line.debit
                        line_credit = line.credit
                        line_pcredit = line.credit
                    print("line_pdebit 1== ", line_pdebit)
                    print("line_pcredit 1== ", line_pcredit)
                    ...
                    line_pdebit = line.company_id.currency_id._convert(line_pdebit, used_currency, company_id, date)
                    line_credit = line.company_id.currency_id._convert(line_credit, used_currency, company_id, date)
                    line_pcredit = line.company_id.currency_id._convert(line_pcredit, used_currency, company_id, date)
                    print("line_pdebit 2== ", line_pdebit)
                    print("line_pcredit 2== ", line_pcredit)
                    ...
                    columns = [{'name': v} for v in [format_date(self.env, line.date), name, partner_name, currency,
                                                     ...
                                                     line_pdebit != 0 and self.format_value(line_pdebit) or '',
                                                     line_pcredit != 0 and self.format_value(line_pcredit) or '',
                                                     ]]
                ...
                if offset == 0:
                    domain_lines.append({
                        ...
                        'columns': [{'name': v} for v in
                                    ['', '', '', amount_currency, self.format_value(debit), self.format_value(credit),
                                     self.format_value(balance), self.format_value(pdebit),
                                     self.format_value(pcredit)]],
                    })
            ...
            lines.append({
                ...
                'columns': [{'name': v} for v in
                            ['', '', '', '', self.format_value(sum_debit), self.format_value(sum_credit),
                             self.format_value(sum_balance), self.format_value(sum_pdebit),
                             self.format_value(sum_pcredit)]],
            })
