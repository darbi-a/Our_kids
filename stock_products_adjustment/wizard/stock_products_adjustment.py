...
class ProductsAdjustment(models.TransientModel):
    _name = 'stock.products.adjustment'
    _description = 'Products Adjustment'
    ...
    def get_report_data(self):
        ...
        if partners:
             ...
             for move in moves:
                  order = self.env['sale.order'].search([('name', '=', move.origin)])
                  partner = order.partner_id
                  if partner.id in partners_ids:
                     st_moves += move
             moves = st_moves and st_moves or moves
        products = moves.mapped('product_id')

        domain = [('id', 'not in', products.ids)]
        domain += seasons and [('season_id', 'in', seasons.ids)]
        domain += categories and [('categ_id', 'in', categories.ids)]
        products = self.env['product.product'].search(domain)
        for product in products:
            qty_available = product.qty_available #aval from that time
            last_po = stock_move.search([('product_id', '=', product.id),
                                         ('location_id.usage', '=', 'supplier')], order='create_date desc', limit=1)
            print(last_po and last_po.date.date())
            last_so = stock_move.search([('product_id', '=', product.id),
                                         ('location_dest_id.usage', '=', 'customer')], order='create_date desc', limit=1)
            data.append({
                 ...
                 'sale_price': qty_available * product.list_price,
    ...
    def action_print_report(self):
        ...
        attachment_model.search(
            [('res_model', '=', 'stock.products.adjustment'), ('res_id', '=', self.id)]).unlink()
        attachment_obj = attachment_model.create({
            'name': xls_file_path,
            'res_model': 'stock.products.adjustment',
            'res_id': self.id,
            'type': 'binary',
