...
class product_pricelist_item(models.Model):
    ...
    @api.one
    @api.depends('categ_id', 'product_tmpl_id','partner_id', 'product_id', 'compute_price', 'fixed_price','pricelist_id', 'percent_price', 'price_discount', 'price_surcharge')
    def _get_pricelist_item_name_price(self):
        if self.categ_id:
            self.name = _("Category: %s") % (self.categ_id.name)
        if self.partner_id:
            self.name = _("Vendor: %s") % (self.partner_id.name)
