...
class KSResConfigSettings(models.TransientModel):
    ...
    line_sales_discount_account = fields.Many2one('account.account', string="Sales Line Discount Account")
    ks_purchase_discount_account = fields.Many2one('account.account', string="Purchase Discount Account")
    line_purchase_discount_account = fields.Many2one('account.account', string="Purchase Line Discount Account")
    ...
    def get_values(self):
        ...
        ks_res.update(
            ...
            line_sales_discount_account=int(self.env['ir.config_parameter'].sudo().get_param('line_sales_discount_account')),
            ks_purchase_discount_account=int(self.env['ir.config_parameter'].sudo().get_param('ks_purchase_discount_account')),
            line_purchase_discount_account=int(self.env['ir.config_parameter'].sudo().get_param('line_purchase_discount_account')),
    ...
    def set_values(self):
        ...
        if self.ks_enable_discount:
            ...
            self.env['ir.config_parameter'].set_param('line_sales_discount_account', self.line_sales_discount_account.id)
            self.env['ir.config_parameter'].set_param('ks_purchase_discount_account',self.ks_purchase_discount_account.id)
            self.env['ir.config_parameter'].set_param('line_purchase_discount_account',self.line_purchase_discount_account.id)